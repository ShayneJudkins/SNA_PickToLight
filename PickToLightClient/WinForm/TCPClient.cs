...
namespace SNA.WinForm.PickToLightClient
{
    public class TCPClient
    {
        ...
        private void Init(IPEndPoint serverIPandPort)
        {
            ...
            {
                _logger.LogError("Check WIFI connection! (Error initiating Network Connection, check EventLog for Exception Mesage.)", "TCPClient.Init() - ArgumentNullException : " + ane.Message.ToString());
            }
            catch (SocketException se)
            {
                _logger.LogError("Check WIFI connection! (Error initiating Network Connection, check EventLog for Exception Mesage.)", "TCPClient.Init() - SocketException : " + se.Message.ToString());
            }
            catch (Exception e)
            {
                _logger.LogError("Check WIFI connection! (Error initiating Network Connection, check EventLog for Exception Mesage.)", "TCPClient.Init() - Unexpected exception : " + e.Message.ToString());
            }
        }
        ...
        public void SendCommand(string command, StringBuilder commandResponse)
        {
            try
            {
                if (_client == null)
                {
                    _logger.LogError("Check WIFI connection! (Connection to Server has been lost, or was never established!)", "Check WIFI connection! (Connection to Server has been lost, or was never established!)");
                    return;
                }
            ...
            {
                _logger.LogError("Check WIFI connection! (Error communicating with Server, check EventLog for Exception Mesage.)", "TCPClient.SendAndReceive() - ArgumentNullException : " + ane.Message.ToString());
            }
            catch (SocketException se)
            {
                _logger.LogError("Check WIFI connection! (Error communicating with Server, check EventLog for Exception Mesage.)","TCPClient.SendAndReceive() - SocketException : " + se.Message.ToString());
            }
            catch (Exception e)
            {
                _logger.LogError("Check WIFI connection! (Error communicating with Server, check EventLog for Exception Mesage.)","TCPClient.SendAndReceive() - Unexpected exception : " + e.Message.ToString());
            }
        }
        ...
        {
            //Let's try NOT catching any Exceptions, so they are all caught in the calling function (only SendCommand() for now)
            int bytesSent = 0;
            ...
            do
            {
                ...
                if (endIndex != -1)
                {
                    ...
                    else
                    {
                        ...
                        _logger.LogError("Check WIFI connection! (Response from Server as in the wrong format, check EventLog for more details.)", "ParseCommandResponse() - Invalid Data Format, received ETX, but no STX!\n\t data = (" + data + ")\n\t processedBuffer = (" + processedBuffer.ToString() + ")");
                ...
                else
                {
                    ...
                    {
                        //Error, no ETX!
                        _logger.LogError("Check WIFI connection! (Response from Server as in the wrong format, check EventLog for more details.)", "ParseCommandResponse() - Invalid Data Format, no ETX!\n\t data = (" + data + ")\n\t processedBuffer = (" + processedBuffer.ToString() + ")");
                    }
                    else
                    {
                        //Error, no ETX!
                        _logger.LogError("Check WIFI connection! (Response from Server as in the wrong format, check EventLog for more details.)","ParseCommandResponse() - Invalid Data Format, no STX or ETX!\n\t data = (" + data + ")\n\t processedBuffer = (" + processedBuffer.ToString() + ")");
                    }
                }
            } while (endIndex != -1);
