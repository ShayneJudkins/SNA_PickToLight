    ...
    //class Logger
    {
        ...
        public void Log(string stringToLog)
        {
            ...
            catch (Exception e)
            {
                ...
                LogError("Error trying to log Errors (Check EventLog for Exception Mesage.)", "Exception in Logger: " + e.Message);
            }
        }
        ...
        public void LogToEventLog(string stringToLog, bool Error)
        {
            using (EventLog eventLog = new EventLog("Application"))
            {
                try
                {
                    ...
                    eventLog.WriteEntry(stringToLog, (Error ? EventLogEntryType.Error : EventLogEntryType.Information));
                }
                catch (Exception ex)
                {
                    Helpers.ShowError("Error writing to Application Event Log, make sure the Source (" + eventLog.Source + ") exists. You may have to create the Source as an Adminstrator. (Exception Message: " + ex.Message + ")");
                }
            }
        }
        ...
        public void LogToSQL(string stringToLog)
        {
            try
            {
                ...
                {
                    LogToEventLog("Error writing to SQL Log Table (" + stringToLog + ")", true);
                    Helpers.ShowError("Error writing to SQL Log Table!");
                }
            }
            catch (SqlException ex)
            {
                ...
                for (int i = 0; i < ex.Errors.Count; i++)
                {
                    ...
                    LogToEventLog("Index #" + i +
                        " Message: " + ex.Errors[i].Message +
                        " LineNumber: " + ex.Errors[i].LineNumber +
                        " Source: " + ex.Errors[i].Source +
                        " Procedure: " + ex.Errors[i].Procedure, true);
                }
                Helpers.ShowError("ERROR writing to SQL Log Table! (Check EventLog for Exception Mesage.)");
            }
            catch (Exception ex)
            {
                if (ex.InnerException != null)
                {
                    ...
                    LogToEventLog("Exception in Logger.LogToSQL() stringToLog(" + stringToLog + "). Exception: " + ex.Message + " (Inner Exception: " + ex.InnerException + ")", true);
                    Helpers.ShowError("ERROR writing to SQL Log Table! (Check EventLog for Exception Mesage.)");
                }
                else
                {
                    ...
                    LogToEventLog("Exception in Logger.LogToSQL() stringToLog(" + stringToLog + "). Exception: " + ex.Message, true);
                    Helpers.ShowError("ERROR writing to SQL Log Table! (Check EventLog for Exception Mesage.)");
                }
            }
        }
        ...
        /// </summary>
        /// <param name="stringToLog"></param>
        public void LogError(string stringToShow, string stringToLog)
        {
            ...
            Helpers.ShowError(stringToShow);
        }
    }
}
